...
class post extends Sximo  {
    ...
    public static function lists( $tag = null) {
        ...
        foreach($rows as $row) {
            ...
            if(file_exists('./uploads/images/'.$row->image) and $row->image !='') {
                $row->image = asset('/uploads/images/'.$row->image);
            } else {
                $row->image = asset('/uploads/images/no-image.png');
            }
            if(file_exists('./uploads/images/'.$row->thumbnail) and $row->thumbnail !='') {
                $row->thumbnail = asset('/uploads/images/'.$row->thumbnail);
            } else {
