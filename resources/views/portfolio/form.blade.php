...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	<div class="col-md-12">
				...
				{!! Form::hidden('id', $row['id']) !!}					
						...
						<div class="image-preview preview-upload">
							{!! SiteHelpers::showUploadedFile( $row["image"],"/public/images/portfolio/") !!}
						</div>
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#category").jCombo("{!! url('portfolio/comboselect?filter=category:id:title') !!}",
		{  selected_value : '{{ $row["category"] }}' });
		
		$("#status").jCombo("{!! url('portfolio/comboselect?filter=permission:id:title') !!}",
