...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	{!! Form::open(array('url'=>'dcsgallary?return='.$return, 'class'=>'form-horizontal validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
	...
	<div class="col-md-12">
				...
				{!! Form::hidden('id', $row['id']) !!}					
						...
						<div class="image-preview preview-upload">
							{!! SiteHelpers::showUploadedFile( $row["image"],"/images/gallery") !!}
						</div>
						...
						required     class='form-control form-control-sm ' /> 
									  ...
									  </div> {!! Form::hidden('cat_btn', $row['cat_btn']) !!}					
									  <div class="form-group row  " >
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#category").jCombo("{!! url('dcsgallary/comboselect?filter=category:id:title') !!}",
		{  selected_value : '{{ $row["category"] }}' });
		
		$("#status").jCombo("{!! url('dcsgallary/comboselect?filter=permission:id:title') !!}",
		{  selected_value : '{{ $row["status"] }}' });
		 	
		 	 

		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("dcsgallary/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
