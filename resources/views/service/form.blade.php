...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	<div class="col-md-12">
						...
						<div class="image-preview preview-upload">
							{!! SiteHelpers::showUploadedFile( $row["image"],"/images/service") !!}
						</div>
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#category").jCombo("{!! url('service/comboselect?filter=category:id:title') !!}",
		{  selected_value : '{{ $row["category"] }}' });
		
		$("#status").jCombo("{!! url('service/comboselect?filter=con_status:id:name') !!}",
		{  selected_value : '{{ $row["status"] }}' });
		
		$("#is_featured").jCombo("{!! url('service/comboselect?filter=isfeatured:id:title') !!}",
