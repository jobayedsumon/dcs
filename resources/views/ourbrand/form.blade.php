...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	{!! Form::open(array('url'=>'ourbrand?return='.$return, 'class'=>'form-horizontal validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
	...
	<div class="p-5">
		...
		<div class="row">
	<div class="col-md-12">
						<fieldset><legend> Ourbrand</legend>
				{!! Form::hidden('id', $row['id']) !!}					
						...
						<div class="image-preview preview-upload">
							{!! SiteHelpers::showUploadedFile( $row["image"],"/images/brand") !!}
						</div>
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#status").jCombo("{!! url('ourbrand/comboselect?filter=permission:id:title') !!}",
		{  selected_value : '{{ $row["status"] }}' });
		 	
		 	 

		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("ourbrand/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
